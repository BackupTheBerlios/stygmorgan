 ...
 if ((fs = fopen (filename, "rb")) != NULL)
     {
        while (!feof(fs))
            {
                i = fread(&nSound, sizeof(nSound),1,fs);
  ...
  if ((fs = fopen (filename, "rb")) != NULL)
       {
               while (!feof(fs))
                 {
                  i = fread(&SongF, sizeof(SongF),1,fs);

                 }

       fclose(fs);

   for(i=0;i<=128;i++)  S[i] = SongF.S[i];
